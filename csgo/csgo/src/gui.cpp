...
bool gui::SetupDirectX() noexcept {
	...
	const auto create = (CreateFn)(GetProcAddress(handle, "Direct3DCreate9"));
...
void gui::SetupMenu(LPDIRECT3DDEVICE9 device) noexcept {
	...
	// We are setting the game's window process to our own WindowProcess created below
	// Then the function returns the original window process
	originalWindowProcess = (WNDPROC)SetWindowLongPtr(window, GWLP_WNDPROC, (LONG_PTR)(WindowProcess));
...
LRESULT CALLBACK WindowProcess(HWND window, UINT message, WPARAM wideParam, LPARAM longParam) {
	// toggle menu
	// ... & 1 - makes it so it only detects it once
